...
function downloadAdminUserReport(user,txs,from,to){
  ...
  const fileName=`moneyflow_${safe}_report_${from}_to_${to}.csv`;
  if(window.MoneyFlowAndroid?.saveCsv){
    const base64=btoa(unescape(encodeURIComponent(csv)));
    if(!window.MoneyFlowAndroid.saveCsv(base64,fileName)) return toast("Could not download report","error");
    return toast("Report downloaded successfully","success");
  }
